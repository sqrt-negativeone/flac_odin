...
decode_flac_file :: proc(data: []u8) {
	flac_stream: flac.Flac_Stream;
	flac.init_flac_stream(&flac_stream, data);
	
	for !flac_stream.done {
		streamed_samples, frames_count := flac.read_samples(&flac_stream, 1024);
...
test_flac_decode_appendix_d_example1 :: proc(t: ^testing.T) {
	decode_flac_file(appendix_d_example1_data);
}

@test
test_flac_decode_appendix_d_example2 :: proc(t: ^testing.T) {
	decode_flac_file(appendix_d_example2_data);
}

@test
test_flac_decode_appendix_d_example3 :: proc(t: ^testing.T) {
	decode_flac_file(appendix_d_example3_data);
}
